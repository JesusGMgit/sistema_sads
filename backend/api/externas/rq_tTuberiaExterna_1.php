...
switch ($request_method) {
    case 'GET':
        // obtener datos de la tabla tuberia externa 1
        if (!empty($_GET["tubo"])) {
...
function get_tubos()
{
    $sql = "SELECT * FROM tuberia_soldadura_externa_1";
    global $conn;
...
{
    $sql = "SELECT * FROM tuberia_soldadura_externa_1 WHERE Tex_ID_tubo=" . $T_ID_tubo . "";
    global $conn;
    $query = $conn->query($sql);
    $data = array();
    while ($r = $query->fetch_assoc()) {
...
function get_tubos_fecha($T_fecha= "")
{
    ...
    $sql = "SELECT * FROM tuberia_soldadura_externa_1 WHERE Tex_Fecha=\"" . $T_fecha . "\"";
...
function get_tubos_proyecto($T_proyecto=0)
{
    ...
    $sql = "SELECT * FROM tuberia_soldadura_externa_1 WHERE Tex_ID_proyecto=\"". $T_proyecto."\"";
...
function insert_tubo()
{
    ...
    $sql = "INSERT INTO tuberia_soldadura_externa_1 (Tex_ID_tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_hora_db,Tex_Archivos_excel,Tex_Observaciones)
...
function update_tubo()
{
    ...
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa_1 SET Tex_Archivos_excel='".$T_Archivos_excel."' WHERE Tex_ID_tubo=".$id_tubo;
...
function update_reporte()
{
    ...
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa_1 SET Tex_Reporte_excel='".$T_nombre_reporte."' WHERE Tex_ID_tubo=".$id_tubo;
...
function update_tuberia(){
    ...
    global $conn;
    $sql = "UPDATE tuberia_soldadura_externa_1 SET Tex_No_tubo='" . $T_no_tubo . "', Tex_No_placa='" . $T_no_placa . "', Tex_ID_proyecto='" . $T_ID_proyecto. "', 
...
function update_hora_db()
{
    ...
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa_1 SET Tex_Hora_db='".$T_hora_db."' WHERE Tex_ID_tubo=".$id_tubo;
...
function delete_tubo($T_ID_tubo)
{
    ...
    $sql = "DELETE FROM tuberia_soldadura_externa_1 WHERE Tex_ID_tubo=" . $T_ID_tubo;
