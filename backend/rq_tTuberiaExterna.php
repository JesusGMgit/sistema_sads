...
function get_tubos()
{
    $sql = "SELECT * FROM tuberia_soldadura_externa";
    global $conn;
...
{
    $sql = "SELECT * FROM tuberia_soldadura_externa WHERE Tex_ID_tubo=" . $T_ID_tubo . "";
    global $conn;
    $query = $conn->query($sql);
    $data = array();
    while ($r = $query->fetch_assoc()) {
...
{
    if($T_maquina!=""){
        $sql = "SELECT Tex_ID_tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_Maquina,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_Archivo_Fexcel
        FROM tuberia_soldadura_externa WHERE Tex_Fecha=\"" . $T_fecha . "\" AND Tex_Maquina=\"".$T_maquina."\"";
    }else{
        $sql = "SELECT Tex_ID_tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_Maquina,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_Archivo_Fexcel
        FROM tuberia_soldadura_externa WHERE Tex_Fecha=\"" . $T_fecha . "\"";
    }
    global $conn;
    $query = $conn->query($sql);
    $data = array();
...
function get_tubos_proyecto($T_proyecto=0,$T_maquina="")
{
    ...
    if($T_maquina!=""){
        $sql = "SELECT Tex_ID_tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_Maquina,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_Archivo_Fexcel
        FROM tuberia_soldadura_externa WHERE Tex_ID_proyecto=\"". $T_proyecto."\" AND Tex_Maquina=\"".$T_maquina."\"";
    }else{
        $sql = "SELECT Tex_ID_tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_Maquina,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_Archivo_Fexcel 
        FROM tuberia_soldadura_externa WHERE Tex_ID_proyecto=\"". $T_proyecto."\"";
    }
    global $conn;
    //echo $sql;
    $query = $conn->query($sql);
...
function insert_tubo()
{
    ...
    $sql = "INSERT INTO tuberia_soldadura_externa (Tex_ID_Tubo,Tex_No_tubo,Tex_No_placa,Tex_ID_proyecto,Tex_Lote_alambre,Tex_Lote_fundente,Tex_Maquina,Tex_FolioOperador,Tex_Fecha,Tex_Hora,Tex_Archivos_excel,Tex_Observaciones)
...
function update_tubo()
{
    ...
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa SET Tex_Archivos_excel='".$T_Archivos_excel."' WHERE Tex_ID_tubo=".$id_tubo;
...
function  update_reporte()
{
    ...
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa SET Tex_Archivo_Fexcel='".$T_nombre_reporte."' WHERE Tex_ID_tubo=".$id_tubo;
...
function update_tuberia(){
    ...
    global $conn;
    $sql = "UPDATE tuberia_soldadura_externa SET Tex_No_tubo='" . $T_no_tubo . "', Tex_No_placa='" . $T_no_placa . "', Tex_ID_proyecto='" . $T_ID_proyecto. "', 
...
function delete_tubo($T_no_tubo)
{
    ...
    $sql = "DELETE FROM tuberia_soldadura_externa WHERE Tex_No_tubo=" . $T_no_tubo;
